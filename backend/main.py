...
def run_full_pipeline() -> List[ResearchItem]:
    ...
    # 2. Source Registry Setup
    registry = SourceRegistry()
    registry.register(ArXivCollector, priority=10, weight=1.0)
    
    enabled_collectors = registry.get_enabled_collectors(config)
    
    # 3. Collector Stage Ingest
    raw_candidates = []
    for col in enabled_collectors:
        try:
            logger.info("Invoking collector: %s", col.name)
            col_items = col.collect()
            raw_candidates.extend(col_items)
        except Exception as e:
            logger.error("Collector '%s' raised an exception: %s", col.name, e)
    ...
    if os.getenv("NVIDIA_API_KEY") and nvidia_config:
        try:
        ...
        logger.info("NVIDIA_API_KEY is absent. Running pipeline in fast local analyzer mode.")
